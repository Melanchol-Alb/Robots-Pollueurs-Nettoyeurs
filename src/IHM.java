...
public class IHM {
	...
	public IHM() {
		...
		String xd = JOptionPane.showInputDialog("Sur quelle ligne se trouve la décharge?");
		String yd = JOptionPane.showInputDialog("Sur quelle colonne se trouve la décharge?");
		try {
		...
		initialize();}
			...
			JOptionPane.showMessageDialog(frame, "Données en entrée non valides!");
		}
	}
	...
	private void initialize() {
		...
		JPanel panelPapsDéchargé = new JPanel();
		panelPapsDéchargé.setBorder(new LineBorder(new Color(0, 0, 0), 1, true));
		panelPapsDéchargé.setBackground(Color.LIGHT_GRAY);
		PanneauHaut.add(panelPapsDéchargé);
		
		lblDéchargé = new JLabel("Papiers gras d\u00E9charg\u00E9s : 0");
		panelPapsDéchargé.add(lblDéchargé);
		...
		JPanel panelCoordonnées = new JPanel();
		panelCoordonnées.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent arg0) {
				
			}
		});
		panelCoordonnées.setBorder(new LineBorder(new Color(0, 0, 0), 2, true));
		panelCoordonnées.setBackground(Color.LIGHT_GRAY);
		PanneauBas.add(panelCoordonnées);
		panelCoordonnées.setLayout(new BorderLayout(0,0));
		...
		PannelUP.add(UP);
		panelCoordonnées.add(PannelUP, BorderLayout.NORTH);
		...
		panelCoordonnées.add(RIGHT, BorderLayout.EAST);
		...
		panelCoordonnées.add(LEFT, BorderLayout.WEST);
		
		JButton btnEvolution = new JButton("Evolution");
		panelCoordonnées.add(btnEvolution, BorderLayout.CENTER);
		
		JPanel panelDOWN = new JPanel();
		panelDOWN.setBackground(Color.LIGHT_GRAY);
		panelCoordonnées.add(panelDOWN, BorderLayout.SOUTH);
		...
		btnEvolution.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				try {
					...
					JOptionPane.showMessageDialog(btnEvolution,"Erreur durant l'évolution !");
					e.printStackTrace();
				}
				try {
					Raffraichir();
		...
		tableNettoyeurs.setModel(new DefaultTableModel(new Object[][] {},new String[] {"Reference", "Surchargé", "Position"}) 
		{private static final long serialVersionUID = 1L;
		public boolean isCellEditable(int row, int column){
			return false;}
		});
		...
		btnSauteur.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				String s = JOptionPane.showInputDialog(btnSauteur," Quel sera la portée du robot sauteur en ("+xSelec+", "+ySelec+")?");
				try {
		...
		btnTleporteur.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				String v = JOptionPane.showInputDialog(btnSauteur," Quel sera la vitesse du Robot Pollueur Téleporteur ?");
				try {
					int i = Integer.parseInt(v);
					if(i<=0)
						throw new NumberFormatException();
				...
				try {
					...
					JOptionPane.showMessageDialog(btnSauteur, "Echec lors de l'ajout du Robot Pollueur Téleporteur.");
				}
			}
		});
		...
		btnTirDiagonal.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				String v = JOptionPane.showInputDialog(btnSauteur," Quel sera la vitesse de tir du Robot Pollueur à Tir Diagonal en ("+xSelec+", "+ySelec+")?");
				try {
					int i = Integer.parseInt(v);
					if(i<=0)
						throw new NumberFormatException();
		...
		btnEssuieGlace.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				String c = JOptionPane.showInputDialog(btnSauteur," Quel sera la capacité du Robot Nettoyeur Essuie Glace en ("+xSelec+", "+ySelec+")?");
				try {
		...
		btnChasseurSauteur.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				String c = JOptionPane.showInputDialog(btnChasseurSauteur," Quel sera la capacité du Robot Nettoyeur Essuie Glace en ("+xSelec+", "+ySelec+")?");
				try {
					int i = Integer.parseInt(c);
					if(i<=0)
						throw new NumberFormatException();
				...
				String s = JOptionPane.showInputDialog(btnChasseurSauteur," Quel sera la portée du robot sauteur en ("+xSelec+", "+ySelec+")?");
				try {
					int i = Integer.parseInt(s);
					if(i<=0)
						throw new NumberFormatException();
		...
		btnNettoyeurToutDroit.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				String c = JOptionPane.showInputDialog(btnNettoyeurToutDroit," Quel sera la capacité du Robot Nettoyeur Tout Droit en ("+xSelec+", "+ySelec+")?");
				try {
					int i = Integer.parseInt(c);
					if(i<=0)
						throw new NumberFormatException();
		...
		btnNettoyeurSpirale.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				String c = JOptionPane.showInputDialog(btnNettoyeurSpirale," Quel sera la capacité du Robot Nettoyeur en Spirale Maximale ?");
				try {
					int i = Integer.parseInt(c);
					if(i<=0)
						throw new NumberFormatException();
				...
				try {
					...
					JOptionPane.showMessageDialog(btnNettoyeurSpirale,"Valeur non valable ! Veuillez donner un entier positif non nul et inférieur à 100!");
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(btnNettoyeurSpirale,"Valeur non valable ! Veuillez donner un entier positif non nul et inférieur au nombre de colonnes!");
				}
				try {
	...
	public void Raffraichir() throws ExceptionCaseExistePas {
		...
		for(RobotNettoyeur r : m.getGestionnaire().Nettoyeurs)
			model.addRow(new Object[] {r.getref(),r.EstPasSurchargé()? "Non":"Oui","("+r.posx+", "+r.posy+")"});
		for(RobotNettoyeur r : m.getGestionnaire().Surchargés)
			model.addRow(new Object[] {r.getref(),r.EstPasSurchargé()? "Non":"Oui","("+r.posx+", "+r.posy+")"});
		...
		for(RobotPollueur r : m.getGestionnaire().Détruits)
			model.addRow(new Object[] {r.getref(),r.estEnVie()? "Oui":"Non","("+r.posx+", "+r.posy+")"});
		lblDéchargé.setText("Papiers gras déchargés : "+m.getPapiersDechargés());
