...
public class RobotNettoyeurSpiraleMaximale extends RobotNettoyeur{
	...
	@Override
	public void Parcourir() throws ExceptionCaseExistePas {//Si il n'y a pas de source trouver une source, sinon parcourir de manière circulaire 2 cases par tour
		if(xSource==-1) {
			TrouverSource();
			caseC=0;
		...
		int x = xSource+itinéraireX.get(caseC), y = ySource+itinéraireY.get(caseC);
		caseC++;
		SeDeplacer(x, y);
		...
		if(caseC==nbCase+1) {//On est arrivés
			xSource = -1;
			rep = 0;
		}
		rep ++;
	...
	private void TrouverItinéraire(int larg) {
